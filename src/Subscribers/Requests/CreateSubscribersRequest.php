...
class CreateSubscribersRequest extends JsonSerializableType
{
    ...
    /**
     * @var ?array<string> $lists List IDs to add subscriber to, covered by subscribers:write. If not provided, a subscriber this call creates follows the workspace default lists setting and an existing subscriber keeps the memberships they already have, so an attribute-only upsert never changes list membership. If empty array, subscriber is added to NO lists.
     */
