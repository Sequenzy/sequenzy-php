...
namespace Sequenzy\Sms;

use Psr\Http\Client\ClientInterface;
use Sequenzy\Core\Client\RawClient;
use Sequenzy\Sms\Types\GetSettingsSmsResponse;
...
use Sequenzy\Sms\Types\UpdateNumberLabelSmsResponse;
...
class SmsClient
{
     ...
     * } $options
     * @return ?GetSettingsSmsResponse
     * @throws SequenzyException
     * @throws SequenzyApiException
     */
    public function getSettings(?array $options = null): ?GetSettingsSmsResponse
    {
     ...
     * @throws SequenzyApiException
     */
    public function sendTest(SendTestSmsRequest $request, ?array $options = null): ?SendTestSmsResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return SendTestSmsResponse::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new SequenzyException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
     ...
     * ```
     *
     * @param string $numberId SMS number ID returned by Get SMS Settings.
     * @param UpdateNumberLabelSmsRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     *   bodyProperties?: array<string, mixed>,
     * } $options
     * @return ?UpdateNumberLabelSmsResponse
     * @throws SequenzyException
     * @throws SequenzyApiException
     */
    public function updateNumberLabel(string $numberId, UpdateNumberLabelSmsRequest $request = new UpdateNumberLabelSmsRequest(), ?array $options = null): ?UpdateNumberLabelSmsResponse
    {
        $options = array_merge($this->options, $options ?? []);
        try {
            $response = $this->client->sendRequest(
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                ...
                return UpdateNumberLabelSmsResponse::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new SequenzyException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (ClientExceptionInterface $e) {
